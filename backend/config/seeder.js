...
const seedUsers = async () => {
  try {
    ...
    const existingStudent = await Student.findOne({ email: 'stu1@example.com' });
    ...
    // Create an admin first (needed for TPO creation)
    const admin = await Admin.findOne();
    if (!admin) {
      throw new Error('Admin account required for seeding');
    }

    let students = [], trainers = [], tpos = [];

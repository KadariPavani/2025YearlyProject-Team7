...
const getAttendanceHistory = async (req, res) => {
  try {
    ...
    // Only generate from the later of: batch startDate, trainer assignedAt, or rangeStart
  ...
  try {
    ...
    if (batch && batch.assignedTrainers) {
      ...
      (batch.assignedTrainers || []).forEach(assignment => {
        ...
        // Skip if requested date is before batch start or trainer assignment
        if (batchStart && reqDate < batchStart) return;
        if (batchEnd && reqDate > batchEnd) return;
        if (assignedAt && reqDate < assignedAt) return;

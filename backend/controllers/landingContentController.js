...
const getPublicLandingContent = async (req, res) => {
  try {
    ...
    console.error('Error fetching public landing content:', err);
...
const getAdminLandingContent = async (req, res) => {
  try {
    ...
    console.error('Error fetching admin landing content:', err);
    res.status(500).json({ success: false, message: 'Server error' });
  }
};
  ...
  try {
    ...
    if (req.file) {
      if (slide.imagePublicId) {
        try { await cloudinary.uploader.destroy(slide.imagePublicId); } catch (e) { console.warn('Could not delete old image:', e.message); }
      }
...
const deleteHeroSlide = async (req, res) => {
  try {
    ...
    if (slide.imagePublicId) {
      try { await cloudinary.uploader.destroy(slide.imagePublicId); } catch (e) { console.warn('Could not delete image:', e.message); }
    }

...
import {
  Mail, Phone, Briefcase, Linkedin, Edit3, Save, X, ArrowLeft,
  BookOpen, Users, CheckSquare, FileText, Shield
} from 'lucide-react';
...
const TrainerProfile = () => {
  ...
  const fetchProfile = async () => {
    ...
    setError('');
    try {
      const res = await api.get('/api/trainer/profile');
      const data = res.data;
      if (data.success) {
        ...
        setFormData({
          ...
          subjectDealing: data.data.subjectDealing || '',
  ...
  const fetchStats = async () => {
    ...
    try {
      const [batchesRes, quizzesRes, assignmentsRes] = await Promise.allSettled([
        api.get('/api/trainer/placement-training-batches'),
        api.get('/api/quizzes'),
      ...
      setStats({
        assignedBatches: Array.isArray(batches) ? batches.length : (batches?.data?.length || 0),
        createdQuizzes: Array.isArray(quizzes) ? quizzes.length : 0,
  ...
  const handleSave = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await api.put('/api/trainer/profile', formData);
  ...
  const handleCancel = () => {
    setFormData({
      ...
      subjectDealing: profile?.subjectDealing || '',
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-6 pt-20 sm:pt-24">
          ...
          <div className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-5">
              ...
              <div className="flex-1 min-w-0 text-center sm:text-left">
                ...
                <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Trainer{profile?.employeeId ? ` · ${profile.employeeId}` : ''}</p>
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 mt-2">
                  ...
                  {profile?.category && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] sm:text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200">
                      {profile.category === 'technical' ? 'Technical' : 'Non-Technical'}
                    </span>
                  )}
                </div>
            ...
            {isEditing && (
              <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs text-gray-500 font-medium mb-1">Subject Dealing</label>
                  <input
                    type="text"
                    name="subjectDealing"
                ...
                <div>
                  <label className="block text-[10px] sm:text-xs text-gray-500 font-medium mb-1">Category</label>
                  <select
                    name="category"
        ...
        <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden mb-4 sm:mb-5">
          <div className="bg-blue-50 px-4 py-2.5 border-b border-gray-200">
            <h2 className="text-xs sm:text-sm font-semibold text-gray-700">Quick Stats</h2>
          </div>
              ...
              <div className="grid grid-cols-3 gap-2.5 sm:gap-4">
                ...
                <div className="bg-white rounded-lg border border-gray-200 p-2.5 sm:p-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] sm:text-xs text-gray-500">Created Quizzes</p>
                      <p className="text-sm sm:text-xl font-bold text-gray-900">{stats?.createdQuizzes ?? 0}</p>
                    </div>
                  </div>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-2.5 sm:p-4">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-[10px] sm:text-xs text-gray-500">Created Assignments</p>
                      <p className="text-sm sm:text-xl font-bold text-gray-900">{stats?.createdAssignments ?? 0}</p>
                    </div>
                  </div>
                </div>
              </div>
